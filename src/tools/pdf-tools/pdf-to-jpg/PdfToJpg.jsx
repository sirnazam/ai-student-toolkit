...
import { AlertTriangle, CheckCircle2, FileImage, Loader2, Upload } from 'lucide-react'
...
export default function PdfToJpg() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await fetch('https://pdf-converter-api-oyer.onrender.com/pdf-to-jpg', {
        method: 'POST',
        body: formData,
      })
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 rounded-full bg-cyan-500/10 px-3 py-1 text-sm font-semibold text-cyan-300">
                <FileImage className="h-4 w-4" /> PDF to JPG
              </p>
              <h1 className="mt-4 text-3xl font-bold tracking-tight text-white text-[var(--text-primary)] sm:text-4xl">PDF to JPG</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-gray-300 text-[var(--text-secondary)] sm:text-base">
                Extract pages from your PDF as JPG images and download them as a ZIP archive.
              </p>
            </div>
          ...
          <section className="rounded-[2rem] border border-dashed border-white/15 bg-white/5 p-6 shadow-2xl shadow-black/20 backdrop-blur-xl sm:p-8">
            ...
            >
              ...
              <div className="space-y-2">
                <p className="text-lg font-semibold text-white">Drag & drop a PDF file here</p>
