...
export default function PdfToWord() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await fetch('https://pdf-converter-api-oyer.onrender.com/pdf-to-word', {
        method: 'POST',
        body: formData,
      })
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 rounded-full bg-cyan-500/10 px-3 py-1 text-sm font-semibold text-cyan-300">
                <FileText className="h-4 w-4" /> PDF to Word
              </p>
              <h1 className="mt-4 text-3xl font-bold tracking-tight text-white text-[var(--text-primary)] sm:text-4xl">PDF to Word</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-gray-300 text-[var(--text-secondary)] sm:text-base">
                Turn your PDF into an editable Word document in seconds.
              </p>
            </div>
